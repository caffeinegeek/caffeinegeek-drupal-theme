...
// Add Zen Tabs styles
if (theme_get_setting('caffeinegeek_tabs')) {
  drupal_add_css( drupal_get_path('theme', 'caffeinegeek') .'/css/tabs.css');
}

function caffeinegeek_preprocess_html(&$vars) {
  global $user, $language;

  // Add role name classes (to allow css based show for admin/hidden from user)
  foreach ($user->roles as $role){
    $vars['classes_array'][] = 'role-' . caffeinegeek_id_safe($role);
  }
  ...
  if (!$vars['is_front']) {
    ...
    $vars['classes_array'][] = caffeinegeek_id_safe('page-'. $path);
    $vars['classes_array'][] = caffeinegeek_id_safe('section-'. $section);
  ...
  // Add IE classes.
  if (theme_get_setting('caffeinegeek_ie_enabled')) {
    $caffeinegeek_ie_enabled_versions = theme_get_setting('caffeinegeek_ie_enabled_versions');
    if (in_array('ie8', $caffeinegeek_ie_enabled_versions, TRUE)) {
...
function caffeinegeek_preprocess_page(&$vars, $hook) {
...
function caffeinegeek_preprocess_node(&$vars) {
  ...
  // Merge first/last class (from caffeinegeek_preprocess_page) into classes array of current node object.
  $node = $vars['node'];
...
function caffeinegeek_preprocess_block(&$vars, $hook) {
  // Add a striping class.
 ...
 */
function caffeinegeek_breadcrumb($variables) {
  $breadcrumb = $variables['breadcrumb'];
  // Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('caffeinegeek_breadcrumb');
  if ($show_breadcrumb == 'yes' || $show_breadcrumb == 'admin' && arg(0) == 'admin') {

    // Optionally get rid of the homepage link.
    $show_breadcrumb_home = theme_get_setting('caffeinegeek_breadcrumb_home');
    ...
    if (!empty($breadcrumb)) {
      $breadcrumb_separator = theme_get_setting('caffeinegeek_breadcrumb_separator');
      $trailing_separator = $title = '';
      if (theme_get_setting('caffeinegeek_breadcrumb_title')) {
        $item = menu_get_item();
      ...
      elseif (theme_get_setting('caffeinegeek_breadcrumb_trailing')) {
        $trailing_separator = $breadcrumb_separator;
      }
 ...
 */
function caffeinegeek_id_safe($string) {
 ...
 *
 */
function caffeinegeek_menu_link(array $variables) {
  ...
  $element['#attributes']['class'][] = caffeinegeek_id_safe($element['#title']);
 ...
 */
function caffeinegeek_preprocess_menu_local_task(&$variables) {
 ...
 */
function caffeinegeek_menu_local_tasks(&$variables) {
  $output = '';
...
/**
 * Duplicate of theme_feed_icon() to change icon.
 */
function caffeinegeek_feed_icon($variables) {
	$text = t('Subscribe to !feed-title', array('!feed-title' => $variables['title']));
	if ($image = theme('image', array('path' => path_to_theme() . '/images/icons/feed.png', 'width' => 16, 'height' => 16, 'alt' => $text))) {
